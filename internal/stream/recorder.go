...
func RecordStream(recordingID int, rawURL string, tunerType string, durationSec int, outputFile string) error {
	...
	if tunerType == "rtsp" {
		...
		// IPTV (Pluto TV, etc.) - We now live-transcode this exactly like HDHomeRun because Pluto TV
		// aggressively obfuscates their ad chunks, making safe proxy-stripping impossible without breaking timestamps.
		args = []string{
			"-user_agent", userAgent,
		}
		if headers != "" {
	...
	// Standard execution for HDHomeRun, RTSP, and IPTV (M3U8 output)
