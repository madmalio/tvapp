...
context_imports = 'import { usePlayer, ChannelInfo, CameraInfo } from "../context/PlayerContext";'
content = re.sub(r'import \{ usePlayer, ChannelInfo \} from "\.\./context/PlayerContext";', context_imports, content)
...
function PinnedCameraOverlay({
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        while (!connected && attempts < 20 && isMounted) {
          attempts++;
          try {
            const res = await fetch(http://:8889//whep, {
              method: "POST",
    ...
    return () => {
      ...
      pc.close();
      if (streamId) {
        fetch(getApiUrl(/api/stream/stop/), { method: "DELETE" }).catch(() => {});
      }
    };
  }, [camera]);
...
content = content.replace('export default function VideoPlayer() {', overlay_code)

state_vars = '''  const { playChannel, cameraPipEnabled, pipCamera, setPipCamera } = usePlayer();
  const [showCameraMenu, setShowCameraMenu] = useState(false);
  const { data: sources } = useApi<CameraInfo[]>("/api/sources");
  const rtspCameras = useMemo(() => sources?.filter(s => s.type === "rtsp") || [], [sources]);'''
content = content.replace('const { playChannel } = usePlayer();', state_vars)
...
overlay_render = '''        />
        ...
        {pipCamera && (
          <PinnedCameraOverlay
            ...
            onExpand={() => navigate(/cameras/)}
          />
        )}'''
content = content.replace('        />', overlay_render)
...
'''
content = content.replace('              {/* Native OS Picture in Picture (Desktop only) */}', button_code + '\n              {/* Native OS Picture in Picture (Desktop only) */}')
