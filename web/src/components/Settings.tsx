      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar p-3 sm:p-6 md:p-8 w-full max-w-full">
        ...
        {activeTab === 'system' && (
          ...
          <div className="bg-neutral-800/50 backdrop-blur-md rounded-2xl p-4 sm:p-6 border border-neutral-700/50 shadow-md">
            ...
            {/* Active Clients */}
            {systemStats?.clients && systemStats.clients.length > 0 && (
              <div className="mt-4 pt-4 border-t border-neutral-700/50">
                <span className="text-xs text-neutral-400 mb-2 block uppercase tracking-wider font-semibold">Active Clients ({systemStats.clients.length})</span>
                <div className="flex flex-wrap gap-2">
                  {systemStats.clients.map((client, idx) => (
                    <span key={idx} className="bg-blue-900/30 text-blue-300 border border-blue-900/50 px-2.5 py-1 rounded-md text-xs font-medium">
                      {client}
                    </span>
                  ))}
                </div>
